                            ...
                            <tbody>
                                @forelse($times as $time)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:void(0)" data-id="{{ $time->id }}" data-name="{{ $time->name }}" data-time="{{ $time->time }}" data-route="{{ route('admin.gamesetting.update', $time->id) }}" data-bs-toggle="modal" data-bs-target="#editModal" class="btn btn-outline--primary editBtn btn-sm me-2"><i class="las la-pen"></i>@lang('Edit')</a>
                                            @if ($time->status)
                                                <button class="btn btn-sm btn-outline--danger confirmationBtn" data-question="@lang('Are you sure to disable this time?')" data-action="{{ route('admin.gamesetting.status', $time->id) }}"><i class="las la-eye-slash"></i>@lang('Disable')</button>
                                            @else
                                                <button class="btn btn-sm btn-outline--success confirmationBtn" data-question="@lang('Are you sure to enable this time?')" data-action="{{ route('admin.gamesetting.status', $time->id) }}"><i class="las la-eye"></i>@lang('Enable')</button>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
    ...
    <div class="modal fade" id="timeModal">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form method="post" action="{{ route('admin.gamesetting.store') }}">
                    @csrf
                    <div class="modal-body">
